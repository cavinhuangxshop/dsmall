...
class Memberfavorites extends BaseMember {
    ...
    public function delfavorites() {
        if (!input('param.fav_id') || !input('param.type')) {
            ds_show_dialog(lang('member_favorite_del_fail'), '', 'error');
        }
        ...
        if (!empty($fav_arr) && is_array($fav_arr)) {
            ...
            if (!empty($favorites_list) && is_array($favorites_list)) {
                ...
                if (!empty($fav_arr) && $result) {
                    ...
                    ds_show_dialog(lang('favorite_del_success'), url('Memberfavorites/fglist', ['show' => input('param.show')]), 'succ');
                }
            } else {
                ds_show_dialog(lang('favorite_del_fail'), '', 'error');
            }
        } else {
            ds_show_dialog(lang('member_favorite_del_fail'), '', 'error');
        }
    ...
    protected function getMemberItemList() {
        $menu_array = array(
            ...
            array(
                'name' => 'fav_flea', 'text' => '收藏闲置', 'url' => url('Memberflea/favorites')
            )
        );
        return $menu_array;
    }
