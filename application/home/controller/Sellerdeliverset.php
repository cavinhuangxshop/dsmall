...
class Sellerdeliverset extends BaseSeller {
    ...
    public function daddress_add() {
        ...
        if ($address_id > 0) {
            ...
            if (!request()->isPost()) {
                ...
                if ($result) {
                    ds_show_dialog(lang('ds_common_op_succ'), 'reload', 'succ', 'CUR_DIALOG.close()');
                } else {
                    ds_show_dialog(lang('store_daddress_modify_fail'), '', 'error');
                }
            }
        } else {
            //新增
            if (!request()->isPost()) {
                ...
                if ($result) {
                    ...
                    ds_show_dialog(lang('store_daddress_add_fail'), '', 'error');
                }
            }
        }
    }
    ...
    public function daddress_del() {
        $address_id = intval(input('param.address_id'));
        if ($address_id <= 0) {
            ds_show_dialog(lang('store_daddress_del_fail'), '', 'error');
        }
        $condition = array();
        ...
        if ($delete) {
            ds_show_dialog(lang('store_daddress_del_succ'), url('Sellerdeliverset/index'), 'succ');
        } else {
            ds_show_dialog(lang('store_daddress_del_fail'), '', 'error');
        }
    }
    ...
    public function free_freight() {
        if (!request()->isPost()) {
            ...
            ds_show_dialog(lang('ds_common_save_succ'), '', 'succ');
        }
    }
    ...
    public function print_set() {
        ...
        if (!request()->isPost()) {
            ...
            if ($rs) {
                ...
                ds_show_dialog(lang('ds_common_save_fail'), '', 'error');
            }
        }
    }
