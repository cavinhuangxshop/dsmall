...
class Membercomplain extends BaseMember {
    ...
    public function _initialize() {
        parent::_initialize(); 
        Lang::load(APP_PATH . 'home/lang/zh-cn/membercomplain.lang.php');
    ...
    public function index() {
        ...
        switch (intval(input('param.select_complain_state'))) {
            case 1:
                $condition['progressing'] = array('lt',90);
                break;
            case 2:
                $condition['finish'] = 99;
                break;
    ...
    public function complain_save() {
        ...
        if ($if_complain < 1) {
            ds_show_dialog(lang('para_error'), url('Memberorder/index'), 'error');
        }
        //检查是不是正在进行投诉
        if ($this->check_complain_exist($input['order_goods_id'])) {
            ds_show_dialog(lang('complain_repeat'), '', 'error');
        }
        ...
        if ($state) {
            ds_show_dialog(lang('complain_submit_success'), url('Membercomplain/index'), 'succ');
        } else {
    ...
    public function complain_add_pic() {
        ...
        if (request()->isPost()) {
            ...
            ds_show_dialog(lang('ds_common_save_succ'), 'reload', 'succ');
        }
    }
    ...
    public function complain_cancel() {
        ...
        if (intval($complain_info['complain_state']) === 10) {
            ...
            ds_show_dialog(lang('complain_cancel_success'), 'reload', 'succ');
        } else {
            ds_show_dialog(lang('complain_cancel_fail'), '', 'error');
        }
    }
