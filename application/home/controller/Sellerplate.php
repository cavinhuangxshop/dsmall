...
class Sellerplate extends BaseSeller {
    ...
    public function plate_add() {
        if (!request()->isPost()) {
            ...
            if (!$validate_result) {
                ds_show_dialog(lang('error') . $validate->getError(), url('Sellerplate/index'));
            }
            //验证数据  END

            $result = model('storeplate')->addStoreplate($insert);
            if ($result) {
                ds_show_dialog(lang('ds_common_op_succ'), url('Sellerplate/index'), 'succ');
            } else {
                ds_show_dialog(lang('ds_common_op_fail'), url('Sellerplate/index'));
            }
        }
    }
