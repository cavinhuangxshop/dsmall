...
class Sellerdeliver extends BaseSeller {
    ...
    public function send() {
        ...
        if (!request()->isPost()) {
            ...
            if (!$result['code']) {
                ...
                ds_show_dialog($result['msg'], url('Sellerorder/index'), 'succ');
            }
        }
    }
    ...
    public function delay_receive() {
        ...
        if (request()->isPost()) {
            ...
            if (!in_array($delay_date, array(5, 10, 15))) {
                ds_show_dialog(lang('wrong_argument'), '', 'error', empty(input('get.inajax')) ? '' : 'CUR_DIALOG.close();');
            }
            ...
            if ($update) {
                ...
                ds_show_dialog("成功将最晚收货期限延迟到了" . $dalay_date . '&emsp;', '', 'succ', empty(input('get.inajax')) ? '' : 'CUR_DIALOG.close();', 4);
            } else {
                ds_show_dialog('延迟失败', '', 'succ', empty(input('get.inajax')) ? '' : 'CUR_DIALOG.close();');
            }
        } else {
    ...
    public function waybill_print() {
        ...
        if (empty($storewaybill_info)) {
            $this->error('请首先绑定打印模板', 'Sellerwaybill/index');
        }
