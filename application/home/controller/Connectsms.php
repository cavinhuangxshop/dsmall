...
class Connectsms extends BaseMall {
    ...
    public function login() {
        ...
        if (request()->isPost()) {
            ...
            if (!empty($member)) {
                ...
                ds_show_dialog('登录成功', $reload, 'succ');
            }
        }
    }
    ...
    public function find_password() {
        ...
        if (config('sms_password') != 1) {
            ds_show_dialog('系统没有开启手机找回密码功能', '', 'error');
        }
        ...
        if ($sms_captcha != session('sms_captcha')) {
            ds_show_dialog('验证码错误', '', 'error');
        }
        if ($sms_mobile != session('sms_mobile')) {
            ds_show_dialog('手机号与接收号不一致', '', 'error');
        }
        ...
        if (!empty($member)) {
            ...
            $member_model->createSession($member); //自动登录
            ds_show_dialog('密码修改成功', url('Memberinformation/index'), 'succ');
        }
    }
