...
class Sellergoodsoffline extends BaseSeller {
    ...
    public function goods_show() {
        ...
        if ($this->store_info['store_state'] != 1) {
            ds_show_dialog(lang('store_goods_index_goods_show_fail') . '，店铺正在审核中或已经关闭', '', 'error');
        }
        ...
        if ($return) {
            ...
            ds_show_dialog(lang('store_goods_index_goods_show_success'), 'reload', 'succ');
        } else {
            ds_show_dialog(lang('store_goods_index_goods_show_fail'), '', 'error');
        }
    }
