...
class Storesnshome extends BaseStoreSns {
    ...
    public function addcomment() {
        ...
        if ($result) {
            ...
            if (input('post.showtype') == 1) {
                $js .="$('#tracereply_" . $stid . "').load(HOMESITEURL+'/Storesnshome/commenttop?id=" . $stid . "');";
            } else {
                $js .="$('#tracereply_" . $stid . "').load(HOMESITEURL+'/Storesnshome/commentlist?id=" . $stid . "');";
            }
    ...
    public function deltrace() {
        ...
        if ($result) {
            ...
            $js = "$('[ds_type=\"tracerow_{$stid}\"]').remove();";
    ...
    private function checkLoginStatus() {
        if (session('is_login') != 1) {
            @header("location: " . HOME_SITE_URL . "/Login/logon.html?ref_url=" . urlencode(url('Membersnshome/index')));
        }
    }
