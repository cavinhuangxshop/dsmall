...
class Goods extends BaseGoods {
    ...
    public function save_consult() {
        ...
        if (!config('guest_comment') && !session('member_id')) {
            ds_show_dialog(lang('goods_index_goods_noallow'));
        }
        ...
        if (trim(input('post.goods_content')) == "") {
            ds_show_dialog(lang('goods_index_input_consult'));
        }
        //表单验证
        ...
        if (!$res) {
            ds_show_dialog($ob_validate->getError());
        }
        ...
        if (empty($goods_info)) {
            ds_show_dialog(lang('goods_index_goods_not_exists'));
        }
        //判断是否是店主本人
        if (session('store_id') && $goods_info['store_id'] == session('store_id')) {
            ds_show_dialog(lang('goods_index_consult_store_error'));
        }
        //检查店铺状态
        ...
        if ($store_info['store_state'] == '0' || intval($store_info['store_state']) == '2' || (intval($store_info['store_endtime']) != 0 && $store_info['store_endtime'] <= time())) {
            ds_show_dialog(lang('goods_index_goods_store_closed'));
        }
        //接收数据并保存
        $input = array();
        ...
        if ($consult_model->addConsult($input)) {
            ds_show_dialog(lang('goods_index_consult_success'), 'reload', 'succ');
        } else {
            ds_show_dialog(lang('goods_index_consult_fail'));
        }
    }
    ...
    public function arrival_notice_submit() {
        ...
        if (!empty($notice_info)) {
            if ($type == 1) {
                ds_show_dialog('您已经添加过通知提醒，请不要重复添加', 'reload');
            } else {
                ds_show_dialog('您已经预约过了，请不要重复预约', 'reload');
            }
        }
