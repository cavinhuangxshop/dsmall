...
class Sellerpromotionmansong extends BaseSeller
{
    ...
    public function mansong_save()
    {
        ...
        if (empty($mansong_name)) {
            ds_show_dialog(lang('mansong_name_error'));
        }
        ...
        if (!check_platform_store()) {
            ...
            $current_mansong_quota = $mansongquota_model->getMansongquotaCurrent(session('store_id'));
            if (empty($current_mansong_quota)) {
                ds_show_dialog(lang('mansong_quota_current_error'), 'reload', 'error');
            }
            ...
            if ($start_time < $quota_start_time) {
                ds_show_dialog(sprintf(lang('mansong_add_start_time_explain'), date('Y-m-d', $current_mansong_quota['mansongquota_starttime'])));
            }
            if ($end_time > $quota_end_time) {
                ds_show_dialog(sprintf(lang('mansong_add_end_time_explain'), date('Y-m-d', $current_mansong_quota['mansongquota_endtime'])));
            }
        }

        if (empty($_POST['mansong_rule'])) {
            ds_show_dialog('满即送规则不能为空');
        }
        ...
        if ($mansong_id) {
            ...
            ds_show_dialog(lang('mansong_add_success'), url('Sellerpromotionmansong/index'), 'succ');
        }
        else {
            ds_show_dialog(lang('mansong_add_fail'));
        }
    }
    ...
    public function mansong_del()
    {
        ...
        if ($result) {
            ...
            ds_show_dialog(lang('ds_common_op_succ'), url('Sellerpromotionmansong/index'), 'succ');
        }
        else {
    ...
    public function mansong_quota_add_save()
    {
        ...
        if ($mansong_quota_quantity <= 0 || $mansong_quota_quantity > 12) {
            ds_show_dialog(lang('mansong_quota_quantity_error'));
        }
        ...
        ds_show_dialog(lang('mansong_quota_add_success'), url('Sellerpromotionmansong/index'), 'succ');
    }
