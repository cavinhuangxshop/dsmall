...
class Memberflea extends BaseFleaMember {
    ...
    public function _initialize() {
        ...
        Lang::load(APP_PATH . 'home/lang/zh-cn/fleacommon.lang.php');
    ...
    public function save_goods() {
        ...
        if (request()->isPost()) {
            ...
            if ($state) {
                ...
                $this->success(lang('store_goods_index_flea_add_success'), 'memberflea/index');
            } else {
                $this->error(lang('store_goods_index_flea_add_fail'));
            }
        }
    }
    ...
    public function flea_del() {
        $res = model('flea')->delFlea(input('goods_id'));
        if ($res) {
    ...
    public function drop_goods() {
        ...
        if ($input_goods_count !== $verify_count) {
            ds_show_dialog(lang('para_error'), '', 'html', 'error');
        }

        $state = $flea_model->delFlea($goods_id);
        if ($state) {
            ds_show_dialog(lang('store_goods_index_flea_del_success'), url('Memberflea/index'), 'succ');
        } else {
            ds_show_dialog(lang('store_goods_index_flea_del_fail'), url('Memberflea/index'), 'error');
        }
    }
    ...
    public function edit_save_goods() {
        ...
        if ($goods_id != 0) {
            ...
            $error = $obj_validate->check($data, $rule);
            if (!$error) {
                $val_error = $obj_validate->getError();
                $this->error(is_array($val_error) ? implode('\n', $val_error) : $val_error);
            }
            /**
             * 实例化闲置物品模型
             */
            ...
            if ($state) {
                ...
                $this->success(lang('store_goods_index_flea_goods_edit_success'), 'memberflea/index');
            } else {
                $this->error(lang('store_goods_index_flea_goods_edit_fail'), 'memberflea/index');
            }
        }
    }
    ...
    public function favorites() {
        ...
        if (input('drop') && input('drop') == 'ok' && input('fav_id')) {
                ...
                foreach ($fav_arr as $fav_id) {
                    if (intval($fav_id) > 0) {
                        if (!$fleafavorites_model->delFleafavorites(intval($fav_id), 'flea')) {
                            ds_show_dialog(lang('flea_favorite_del_fail'), '', 'error');
                        }
                    }
                }
            ...
            ds_show_dialog(lang('flea_favorite_del_success'), url('Memberflea/favorites'), 'succ');
        }
        /**
         * 分页
         */
         ...
         * 得到收藏列表
         */
        $favorites_list = $fleafavorites_model->getFleafavoritesList(array('member_id' => session('member_id'), 'fav_type' => 'flea'), 10);
