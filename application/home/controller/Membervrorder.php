...
class Membervrorder extends BaseMember {
    ...
    public function index() {
        ...
        foreach ($order_list as $key => $order) {
            ...
            //显示分享
            $order_list[$key]['if_share'] = $vrorder_model->getVrorderOperateState('share', $order);
    ...
    public function show_order() {
        ...
        //显示分享
        $order_info['if_share'] = $vrorder_model->getVrorderOperateState('share', $order_info);
    ...
    public function resend() {
        ...
        if (empty($order_info) && $order_info['order_state'] != ORDER_STATE_PAY) {
            ds_show_dialog('订单信息发生错误');
        }
        if ($order_info['vr_send_times'] >= 5) {
            ds_show_dialog('您发送的次数过多，无法发送');
        }
        ...
        ds_show_dialog('发送成功', '', 'succ', "DialogManager.close('vr_code_resend');");
    }
    ...
    protected function getMemberItemList() {
        ...
        if(request()->action()=='show_order'){
            $menu_array[] = array(
                'name' => 'show_order', 'text' => lang('订单详情'),
