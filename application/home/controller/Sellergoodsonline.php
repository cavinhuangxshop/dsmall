...
class Sellergoodsonline extends BaseSeller {
    ...
    public function edit_save_goods() {
        ...
        if (!request()->isPost() || $common_id <= 0) {
            ds_show_dialog(lang('store_goods_index_goods_edit_fail'), url('Sellergoodsonline/index'));
        }
        ...
        if (!isset($data[$gc_id]) || isset($data[$gc_id]['child']) || isset($data[$gc_id]['childchild'])) {
            ds_show_dialog(lang('store_goods_index_again_choose_category1'));
        }
            ...
            if (empty($bind_info)) {
                ...
                if (empty($bind_info)) {
                    ...
                    if (empty($bind_info)) {
                        ...
                        $bind_info = $storebindclass_model->getStorebindclassInfo($where);
                        if (empty($bind_info)) {
                            ds_show_dialog(lang('store_goods_index_again_choose_category2'));
                        }
                    }
                }
            }
        ...
        //if ($return>=0) {
            //提交事务
            model('goods')->commit();
            ds_show_dialog(lang('ds_common_op_succ'), url('Sellergoodsonline/index'), 'succ');
        /*} else {
            //回滚事务
    ...
    public function edit_save_image() {
        if (request()->isPost()) {
            ...
            if ($common_id <= 0 || empty($_POST['img'])) {
                ds_show_dialog(lang('wrong_argument'), url('Sellergoodsonline/index'));
            }
            $goods_model = model('goods');
            // 删除原有图片信息
            ...
            if ($rs) {
                ...
                ds_show_dialog(lang('ds_common_save_fail'), url('Sellergoodsonline/index'));
            }
        }
    }
    ...
    public function drop_goods() {
        ...
        if ($return) {
            ...
            ds_show_dialog(lang('store_goods_index_goods_del_success'), 'reload', 'succ');
        } else {
            ds_show_dialog(lang('store_goods_index_goods_del_fail'), '', 'error');
        }
    }
    ...
    public function goods_unshow() {
        ...
        if ($return) {
            ...
            ds_show_dialog(lang('store_goods_index_goods_unshow_success'), get_referer() ? get_referer() : url('Sellergoodsonline/goods_list'), 'succ', '', 2);
        } else {
            ds_show_dialog(lang('store_goods_index_goods_unshow_fail'), '', 'error');
        }
    }
    ...
    private function checkRequestCommonId($common_ids) {
        if (!preg_match('/^[\d,]+$/i', $common_ids)) {
            ds_show_dialog(lang('para_error'), '', 'error');
        }
        return $common_ids;
    }
    ...
    public function maker_qrcode() {
        ...
        $lst = $goods_model->getGoodsList($where, 'goods_id');
        if (empty($lst)) {
            echo '未找到商品信息';
            retrun;
        }
        foreach ($lst as $k => $v) {
            $goods_id = $v['goods_id'];
            $qrcode_url = WAP_SITE_URL . '/mall/product_detail.html?goods_id=' . $goods_id;
        ...
        //生成店铺二维码
        $qrcode_url = WAP_SITE_URL . '/mall/store.html?store_id=' . $store_id;
        $PhpQRCode->set('date', $qrcode_url);
