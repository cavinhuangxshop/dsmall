...
class Sellerpromotionxianshi extends BaseSeller
{
    ...
    public function xianshi_save()
    {
        ...
        if (empty($xianshi_name)) {
            ds_show_dialog(lang('xianshi_name_error'));
        }
        ...
        if (!check_platform_store()) {
            ...
            $current_xianshi_quota = $xianshiquota_model->getXianshiquotaCurrent(session('store_id'));
            if (empty($current_xianshi_quota)) {
                ds_show_dialog('没有可用限时折扣套餐,请先购买套餐');
            }
            ...
            if ($start_time < $quota_start_time) {
                ds_show_dialog(sprintf(lang('xianshi_add_start_time_explain'), date('Y-m-d', $current_xianshi_quota['xianshiquota_starttime'])));
            }
            if ($end_time > $quota_end_time) {
                ds_show_dialog(sprintf(lang('xianshi_add_end_time_explain'), date('Y-m-d', $current_xianshi_quota['xianshiquota_endtime'])));
            }
        }
        ...
        if ($result) {
            ...
            ds_show_dialog(lang('xianshi_add_success'), url('Sellerpromotionxianshi/xianshi_manage','xianshi_id='.$result), 'succ', '', 3);
        }
        else {
            ds_show_dialog(lang('xianshi_add_fail'));
        }
    }
    ...
    public function xianshi_edit_save()
    {
        ...
        if ($result && $result) {
            ...
            ds_show_dialog(lang('ds_common_op_succ'), url('Sellerpromotionxianshi/index'), 'succ', '', 3);
        }
        else {
    ...
    public function xianshi_del()
    {
        ...
        if ($result) {
            ...
            ds_show_dialog(lang('ds_common_op_succ'), url('Sellerpromotionxianshi/index'), 'succ');
        }
        else {
            ds_show_dialog(lang('ds_common_op_fail'));
        }
    ...
    public function xianshi_quota_add_save()
    {
        ...
        if ($xianshi_quota_quantity <= 0 || $xianshi_quota_quantity > 12) {
            ds_show_dialog(lang('xianshi_quota_quantity_error'));
        }
        ...
        ds_show_dialog(lang('xianshi_quota_add_success'), url('Sellerpromotionxianshi/index'), 'succ');
    }
    ...
    public function xianshi_goods_price_edit()
    {
        ...
        if ($result) {
            ...
            $this->recordSellerlog('限时折扣价格修改为：' . $xianshigoods_info['xianshigoods_price'] . '，商品名称：' . $xianshigoods_info['goods_name']);
        }
        else {
            $data['result'] = false;
    ...
    public function xianshi_goods_delete()
    {
        ...
        $this->recordSellerlog('删除限时折扣商品，活动名称：' . $xianshi_info['xianshi_name'] . '，商品名称：' . $xianshigoods_info['goods_name']);
        echo json_encode($data);
        die;
    }
