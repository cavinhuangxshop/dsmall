...
class Membervrrefund extends BaseMember
{
    ...
    public function add_refund()
    {
        ...
        if ($order_id < 1) {//参数验证
            ds_show_dialog(lang('wrong_argument'), url('Membervrrefund/index'), 'error');
        }
        $condition = array();
        ...
        if (!$order['if_refund']) {//检查状态,防止页面刷新不及时造成数据错误
            ds_show_dialog(lang('wrong_argument'), url('Membervrorder/index'), 'error');
        }
        if (request()->isPost() && $order['if_refund']) {
            ...
            if ($state) {
                ds_show_dialog(lang('ds_common_save_succ'), url('Membervrrefund/index'), 'succ');
            } else {
