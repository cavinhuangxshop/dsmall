...
class Sellerpromotionmgdiscount extends BaseSeller
{
    ...
    {
        parent::_initialize();
        Lang::load(APP_PATH . 'home/lang/zh-cn/sellerpromotionmgdiscount.lang.php');
        ...
        if (!request()->isPost()) {
            ...
            foreach ($membergrade_arr as $key => $value) {
                ...
                if($current_level_discount>$pre_level_discount){
                    ds_show_dialog($value['level_name'].'的折扣应小于或等于'.($pre_level_discount/10).'折');
                }
                $pre_level_discount = $current_level_discount;
                    
                if ($current_level_discount < 1 || $current_level_discount > 100) {
                    ds_show_dialog(lang('输入的折扣不正确'));
                }
            ...
            ds_show_dialog(lang('店铺会员折扣设置成功'), url('Sellerpromotionmgdiscount/mgdiscount_store'), 'succ', '', 3);
        }
    }
        ...
        if (!request()->isPost()) {
            ...
            if (!check_platform_store()) {
                ...
                if (empty($current_mgdiscount_quota)) {
                    ds_show_dialog('没有可用会员等级折扣套餐,请先购买套餐');
                }
                ...
                if (TIMESTAMP < $quota_start_time) {
                    ds_show_dialog(sprintf(lang('mgdiscount_add_start_time_explain'), date('Y-m-d', $current_mgdiscount_quota['mgdiscountquota_starttime'])));
                }
                if (TIMESTAMP > $quota_end_time) {
                    ds_show_dialog(sprintf(lang('mgdiscount_add_end_time_explain'), date('Y-m-d', $current_mgdiscount_quota['mgdiscountquota_endtime'])));
                }
            }
            ...
            if ($result&&$result1) {
                ...
                ds_show_dialog(lang('mgdiscount_add_success'), url('Sellerpromotionmgdiscount/mgdiscount_goods'), 'succ', '', 3);
            } else {
                ds_show_dialog(lang('mgdiscount_add_fail'));
            }
        }
    }
    ...
    function mgdiscount_goods_edit()
    {
        ...
        if (empty($goodscommon_info) || $goodscommon_info['store_id'] != session('store_id')) {
            ds_show_dialog(lang('您的商品不存在，或商品已被锁定，请联系管理员删除抢购解除锁定'));
        }
    ...
    public function mgdiscount_del()
    {
        ...
        if ($result && $result1) {
            ds_show_dialog(lang('ds_common_op_succ'), url('Sellerpromotionmgdiscount/mgdiscount_goods'), 'succ');
        } else {
    ...
    public function mgdiscount_quota_add_save()
    {
        ...
        if ($mgdiscount_quota_quantity <= 0 || $mgdiscount_quota_quantity > 12) {
            ds_show_dialog(lang('mgdiscount_quota_quantity_error'));
        }
        //获取当前价格
        ...
        ds_show_dialog(lang('mgdiscount_quota_add_success'), url('Sellerpromotionmgdiscount/mgdiscount_store'), 'succ');
    }
