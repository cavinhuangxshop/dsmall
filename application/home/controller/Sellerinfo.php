...
class Sellerinfo extends BaseSeller
{
    ...
    public function bind_class_save()
    {
        ...
        if ($result) {
            ds_show_dialog('申请成功，请等待系统审核', url('Sellerinfo/bind_class'), 'succ', input('param.inajax') ? '' : 'CUR_DIALOG.close();');
        }
        else {
            ds_show_dialog(url('ds_common_save_fail'));
        }
    }
    ...
    public function reopen_add() {
        if (request()->isPost()) {
            ...
            if (empty($store_info['store_endtime'])) {
                ds_show_dialog('您的店铺使用期限无限制，无须续签');
            }
            ...
            if ($storereopen_model->getStorereopenCount($condition)) {
                ds_show_dialog('目前尚存在申请中的续签信息，不能重复申请');
            }
            ...
            if ($insert) {
                if ($data['storereopen_pay_amount'] == 0) {
                    ds_show_dialog('您的申请已经提交，请等待管理员审核', 'reload', 'succ', '', 5);
                } else {
                    ds_show_dialog(lang('ds_common_save_succ') . '，需付款金额' . ds_price_format($data['storereopen_pay_amount']) . '元，请尽快完成付款，付款完成后请上传付款凭证', 'reload', 'succ', '', 5);
                }
            } else {
                ds_show_dialog(lang('ds_common_del_fail'));
            }
    ...
    public function reopen_upload()
    {
        ...
        if ($update) {
            ds_show_dialog('上传成功，请等待系统审核', 'reload', 'succ');
        }
        else {
            ds_show_dialog(lang('ds_common_del_fail'));
        }
