...
class Sellergroupbuy extends BaseSeller
{
    ...
    public function groupbuy_quota_add_save()
    {
        ...
        ds_show_dialog(lang('groupbuy_quota_add_success'), url('Sellergroupbuy/index'), 'succ');
    }
...
//            halt($current_groupbuy_quota);
    ...
    public function groupbuy_save()
    {
        ...
        if (!check_platform_store()) {
            ...
            $current_groupbuy_quota = $groupbuyquota_model->getGroupbuyquotaCurrent(session('store_id'));
            if (empty($current_groupbuy_quota)) {
                ds_show_dialog('当前没有可用套餐，请先购买套餐', url('Store_groupbuy/groupbuy_quota_add'));
            }
        }
        ...
        if (input('param.vr')) {
            if ($param['groupbuy_upper_limit'] > 0 && $goods_info['virtual_limit'] > 0 && $param['groupbuy_upper_limit'] > $goods_info['virtual_limit']) {
                ds_show_dialog(sprintf('虚拟抢购活动的限购数量(%d)不能大于虚拟商品本身的限购数量(%d)', $param['groupbuy_upper_limit'], $goods_info['virtual_limit']), url('Sellergroupbuy/index'));
            }
        ...
        if ($result) {
            ...
            ds_show_dialog(lang('groupbuy_add_success'), url('Sellergroupbuy/index'), 'succ');
        }
        else {
            ds_show_dialog(lang('groupbuy_add_fail'), url('Sellergroupbuy/index'));
        }
    }
