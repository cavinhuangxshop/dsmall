...
class Selleractivity extends BaseSeller
{
    ...
    public function activity_apply_save()
    {
        ...
        if (empty($_POST['item_id'])) {
            ds_show_dialog(lang('store_activity_choose_goods'), url('Selleractivity/index'));
        }
        $activity_id = intval(input('post.activity_id'));
        if ($activity_id <= 0) {
            ds_show_dialog(lang('para_error'), url('Selleractivity/index'));
        }
        ...
        if (empty($activity) || $activity['activity_type'] != '1' || $activity['activity_state'] != '1' || $activity['activity_startdate'] > time() || $activity['activity_enddate'] < time()) {
            ds_show_dialog(lang('store_activity_not_exists'), url('Selleractivity/index'));
        }
        $activitydetail_model = model('activitydetail');
        ...
        foreach ($_POST['item_id'] as $item_id) {
            ...
            elseif (in_array($item_id, $ids_state2)) {
                ...
                $activitydetail_model->updateActivitydetailList($input, array('item_id' => $item_id));
            }
        }
        ds_show_dialog(lang('store_activity_submitted'), 'reload', 'succ');
    }
