...
class Sellersetting extends BaseSeller {
        ...
        if (request()->isPost()) {
            ...
            if (!empty(input('post.store_name'))) {
                ...
                if (!empty($store) && ($store_id != $store['store_id'])) {
                    ds_show_dialog('此店铺名称已被其它人使用，请换其它名称。', 'reload', 'error');
                    return;
                }
            ...
            $PhpQRCode->set('date', WAP_SITE_URL . '/mall/store.html?store_id=' . $store_id);
            $PhpQRCode->set('pngTempName', $store_id . '_store.png');
            $PhpQRCode->init();

            $this->getMiniProCode(1);
            $store_model->editStore($param, array('store_id' => $store_id));
            ds_show_dialog(lang('ds_common_save_succ'), url('Sellersetting/setting'), 'succ');
        }
        /**
         * 实例化店铺等级模型
         */
    ...
    public function store_slide() {
        ...
        if (request()->isPost()) {
            ...
            ds_show_dialog(lang('ds_common_save_succ'), url('Sellersetting/store_slide'), 'succ');
        } else {
            // 删除upload中的无用数据
    ...
    public function set_theme() {
        ...
        if (!empty($style) && file_exists(PUBLIC_PATH . '/static/home/default/store/styles/theme/' . $style . '/images/preview.jpg')) {
            ...
            ds_show_dialog(lang('store_theme_congfig_success'), 'reload', 'succ');
        } else {
            ds_show_dialog(lang('store_theme_congfig_fail'), '', 'error');
        }
    }
        ...
        if (request()->isPost()) {
            ...
            if (!empty($_FILES['mb_title_img']['name'])) {
                ...
                if ($result) {
                    ...
                    ds_show_dialog($file_object->getError());
                }
            }
            ...
            ds_show_dialog('保存成功', url('Sellersetting/store_mobile'), 'succ');
        }
    ...
    public function map() {
        ...
        if (request()->isPost()) {
            ...
            ds_show_dialog('保存成功', url('Sellersetting/map'), 'succ');
        }
        $this->assign('store_info', $store_info);
